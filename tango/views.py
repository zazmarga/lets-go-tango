...
class ActivitiesListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        queryset = (Activity.objects.order_by("name")
                    .select_related("location", "possessor"))
        ...
        if form.is_valid():
            ...
            if form.cleaned_data.get("is_mine"):
                queryset = queryset.filter(
                    possessor=form.cleaned_data.get("user_id")
                )
            if day_of_week != "":
...
class ActivityDetailView(LoginRequiredMixin, generic.DetailView):
    model = Activity
    queryset = (Activity.objects.order_by("name").
                select_related("category", "location", "possessor"))
    ...
    def post(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(opinion_form=form)
        )
...
class MembersListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["occupations"] = Occupation.objects.annotate(
...
class MemberDetailView(LoginRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["activities"] = (Activity.objects.order_by("name")
                                 .select_related("possessor"))
        return context
...
class PlacesListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["cities_count"] = Place.objects.values("city").annotate(
